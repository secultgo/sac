...
Route::prefix('painel')->middleware('auth')->group(function () {
    ...
    /**
     * Rotas abertas para qualquer usuário autenticado (sem restrição de nível)
     */
    ...
    Route::get('chamados', fn() => redirect()->route('painel.dashboard'))->name('chamados.index');

    // Meus atendimentos
     Route::get('meus-atendimentos', [ChamadoController::class, 'meusAtendimentos'])->name('meus-atendimentos.index');

    /**
     * Rotas privadas (precisam de autenticação E nível)
     */
    Route::middleware('nivel.atendimento')->group(function () {
        ...
        // Equipe e avaliações
        Route::get('equipe', [UserController::class, 'equipe'])->name('equipe.index')->middleware('can:gestor');
        Route::get('avaliacoes', [UserController::class, 'avaliacoes'])->name('avaliacoes.index')->middleware('can:gestor');
        Route::post('avaliacoes/{chamado}/ciente', [UserController::class, 'marcarCiente'])->name('avaliacoes.ciente')->middleware('can:gestor');
    });
});
